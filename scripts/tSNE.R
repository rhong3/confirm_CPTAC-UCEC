...
inlist=c('F2POLE_NL5', 'X4CNVH_NL5', 'F4his_NL5')

for(xx in inlist){
  input_file=paste('~/documents/confirm_CPTAC-UCEC/Results/',xx,'/out/For_tSNE.csv',sep='')
  output_file=paste('~/documents/confirm_CPTAC-UCEC/Results/',xx,'/out/tSNE_P_N.csv',sep='')
  sampled_file=paste('~/documents/confirm_CPTAC-UCEC/Results/',xx,'/out/tSNE_sampled.csv',sep='')
  out_fig=paste('~/documents/confirm_CPTAC-UCEC/Results/',xx,'/out/P_N.pdf',sep='')
  start=12
  bins=50

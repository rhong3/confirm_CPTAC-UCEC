...
for (i in fts){
  for (j in ars){
    print(paste(j, i, sep=""))
    inlist = append(inlist, paste(j, i, sep=""))
  }
}

# Check previously calculated trials
previous=read.csv("~/Documents/confirm_CPTAC-UCEC/Results/Statistics_confirmatory_new.csv")
existed=paste(previous$Architecture, previous$Feature, sep='')
# Find the new trials to be calculated
targets = inlist[which(!inlist %in% existed)]
OUTPUT = setNames(data.frame(matrix(ncol = 50, nrow = 0)), c("Feature", "Architecture", "Patient_ROC.95.CI_lower", "Patient_ROC",                 
  ...
  tryCatch(
    {
      ...
      # Key names
      keydf = data.frame("Feature" = feature, "Architecture" = arch)
...
New_OUTPUT = New_OUTPUT[order(New_OUTPUT$Feature, -New_OUTPUT$Patient_ROC),]
write.csv(New_OUTPUT, file = "~/documents/confirm_CPTAC-UCEC/Results/Statistics_confirmatory_new.csv", row.names=FALSE)

...
def slide_metrics(inter_pd, path, name, fordict, pmd):
    ...
    if pmd == 'subtype':
        inter_pd['Prediction'] = inter_pd[
            ['POLE_score', 'MSI.H_score', 'CNV.L_score', 'CNV.H_score']].idxmax(axis=1)
        redict = {'MSI.H_score': int(1), 'CNV.L_score': int(2), 'CNV.H_score': int(3), 'POLE_score': int(0)}
    elif pmd == 'histology':
        inter_pd['Prediction'] = inter_pd[
    ...
    try:
        ...
        if pmd == 'subtype':
            pdx_slide = inter_pd[['POLE_score', 'MSI.H_score', 'CNV.L_score', 'CNV.H_score']].values
        elif pmd == 'histology':
...
def realout(pdx, path, name, pmd):
    if pmd == 'subtype':
        lbdict = {1: 'MSI.H', 2: 'CNV.L', 3: 'CNV.H', 0: 'POLE'}
    elif pmd == 'histology':
    ...
    if pmd == 'subtype':
        out = pd.DataFrame(pdx, columns=['POLE_score', 'MSI.H_score', 'CNV.L_score', 'CNV.H_score'])
    elif pmd == 'histology':
...
def metrics(pdx, tl, path, name, pmd, ori_test=None):
    ...
    if pmd == 'subtype':
        lbdict = {1: 'MSI.H', 2: 'CNV.L', 3: 'CNV.H', 0: 'POLE'}
        outt = pd.DataFrame(pdxt, columns=['POLE_score', 'MSI.H_score', 'CNV.L_score', 'CNV.H_score'])
    elif pmd == 'histology':
        lbdict = {0: 'Endometrioid', 1: 'Serous'}
...
def CAM(net, w, pred, x, y, path, name, bs, pmd, rd=0):
    if pmd == 'subtype':
        ...
        DIRB = "../Results/{}/out/{}_CNV.L_img".format(path, name)
        DIRC = "../Results/{}/out/{}_CNV.H_img".format(path, name)
        ...
        for DIR in (DIRA, DIRB, DIRC, DIRD):
            try:
                ...
                pass
        catdict = {1: 'MSI.H', 2: 'CNV.L', 3: 'CNV.H', 0: 'POLE'}

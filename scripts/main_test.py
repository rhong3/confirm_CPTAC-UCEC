    ...
    if opt.mode in ['X1', 'X2', 'X3', 'X4', 'F1', 'F2', 'F3', 'F4']:
        ...
        try:
            ...
            if opt.pdmd == 'histology':
                ...
                for idx, row in ref.iterrows():
                    big_images.append(
                        [row['Patient_ID'], int(row[opt.pdmd]), img_dir + "{}/".format(str(row['Patient_ID'])), row['age'], row['BMI']])
        ...
        try:
            ...
            if opt.pdmd == 'histology':
                ...
                Serousimg = Sample_prep.intersection(ref.loc[ref['histology_Serous'] == 1]['Patient_ID'].tolist(), allimg)
                for i in EMimg:
                ...
                negimg = Sample_prep.intersection(ref.loc[ref['subtype_{}'.format(opt.pdmd)] == 0]['Patient_ID'].tolist(), allimg)
                posimg = Sample_prep.intersection(ref.loc[ref['subtype_{}'.format(opt.pdmd)] == 1]['Patient_ID'].tolist(), allimg)
                for i in negimg:
                    big_images.append([i, level, img_dir + "{}/level{}".format(i, level), 0])
                for i in posimg:
                    big_images.append([i, level, img_dir + "{}/level{}".format(i, level), 1])
            else:
                negimg = Sample_prep.intersection(ref.loc[ref[opt.pdmd] == 0]['Patient_ID'].tolist(), allimg)
                posimg = Sample_prep.intersection(ref.loc[ref[opt.pdmd] == 1]['Patient_ID'].tolist(), allimg)
                for i in negimg:
                    big_images.append([i, level, img_dir + "{}/level{}".format(i, level), 0])
                for i in posimg:
                    big_images.append([i, level, img_dir + "{}/level{}".format(i, level), 1])
